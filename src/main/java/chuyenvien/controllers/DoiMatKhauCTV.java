...
/**
 * Servlet implementation class DoiMatKhauCTV
 */
@WebServlet("/DoiMatKhauCTV")
public class DoiMatKhauCTV extends HttpServlet {
	private static final long serialVersionUID = 1L;

    public DoiMatKhauCTV() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String user=account.getUsernameString();
		...
		{
			String ms="Mật khẩu mới và mật khấu nhập lại không trùng!!!";
			request.setAttribute("ms", ms);
			request.getRequestDispatcher("ThongTinCTV.jsp").forward(request, response);
		}
		else
		{
			dao.UpdatePasswordCTV(user, rpass);
